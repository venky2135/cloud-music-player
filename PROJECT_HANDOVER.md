...
14. **Resolved Render Container JS Runtime Mismatch (Deno + Node 22)** — Debian Bookworm's default `apt-get install nodejs` installs Node v18, which is rejected by yt-dlp (`minimum supported version: 22.0.0`), causing `"The page needs to be reloaded"` on Render. Fixed by adding official standalone Deno (`COPY --from=denoland/deno:bin /deno /usr/local/bin/deno`) and NodeSource Node.js 22 LTS to `Dockerfile` and `backend/Dockerfile`. Committed and pushed (`fef86db`) to GitHub to trigger fresh Render deployment.
